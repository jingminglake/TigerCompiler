...
public class TranslateVisitor implements ast.Visitor {
    ...
    public boolean flag;//标记是否为new自动生成id
    public String class_type;//标记当前访问的类的id
	...
	public void visit(ast.Ast.Exp.Call e) {
		this.flag = false;//call会给new加id，所以访问到new时不需要加id
		e.exp.accept(this);
		this.flag = true;
	...
	public void visit(ast.Ast.Exp.NewIntArray e) {
		...
		if (flag) {
			String newid = this.genId();
			this.tmpVars.add(new Dec.DecSingle(new Type.ClassType(this.class_type), newid));
		}
		return;
	}
	...
	public void visit(ast.Ast.Stm.AssignArray s) {
		...
		s.exp.accept(this);
		this.flag = true;
		this.stm = new AssignArray(s.id, index, this.exp, s.isField);
		return;
	}
	...
	public void visit(ast.Ast.Program.ProgramSingle p) {
		...
		this.flag = true;//每次处理一个程序前，打开标志位
		scanProgram(p);
